 ...
 */
// ============================================================
// Section 0.  Bettbox Compatibility & Group Switches
// ============================================================
...
const ruleOptionsEnable = {
    ...
    "Latency Test": true,
    "Failover": true,
    "Load Balance (Hash)": true,
    "Load Balance (Round Robin)": true,
};

const serviceConfigs = [
    { name: "HK - 香港",       icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/flags/hk.svg", fallback: "Others" },
    { name: "JP - 日本",       icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/flags/jp.svg", fallback: "Others" },
    { name: "US - 美国",       icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/flags/us.svg", fallback: "Others" },
    { name: "SG - 新加坡",     icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/flags/sg.svg", fallback: "Others" },
    ...
    { name: "Global Block",    icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/block.svg", fallback: "DIRECT" },
    { name: "Latency Test",    icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/speed.svg", fallback: "Select Node" },
    { name: "Failover",        icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/ambulance.svg", fallback: "Select Node" },
    { name: "Load Balance (Hash)",        icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/merry_go.svg", fallback: "Select Node" },
    { name: "Load Balance (Round Robin)", icon: "https://fastly.jsdelivr.net/gh/clash-verge-rev/clash-verge-rev.github.io@main/docs/assets/icons/balance.svg",  fallback: "Select Node" },
];

const BUILTIN_TARGETS = new Set(["DIRECT", "REJECT", "PASS"]);
...
// ============================================================
// Section 5b.  Bettbox helpers
// ============================================================

function _buildEnabledSet() {
    var enabled = new Set();
    enabled.add("Select Node");
    enabled.add("Fallback");
    if (typeof ruleOptionsEnable === "object" && ruleOptionsEnable) {
        var keys = Object.keys(ruleOptionsEnable);
        for (var i = 0; i < keys.length; i++) {
            if (ruleOptionsEnable[keys[i]]) enabled.add(keys[i]);
        }
    }
    return enabled;
}

function _buildFallbackMap() {
    var map = new Map();
    if (Array.isArray(serviceConfigs)) {
        for (var i = 0; i < serviceConfigs.length; i++) {
            var svc = serviceConfigs[i];
            if (svc && svc.name && svc.fallback) {
                map.set(svc.name, svc.fallback);
            }
        }
    }
    return map;
...
function _resolveTarget(target, enabledSet, fallbackMap) {
    var visited = new Set();
    var current = target;
    while (current) {
        if (BUILTIN_TARGETS.has(current) || enabledSet.has(current)) {
            return current;
        }
...
function _rewriteRuleTargets(rules, enabledSet, fallbackMap) {
    return rules.map(function (rule) {
        ...
        for (var i = parts.length - 1; i >= 0; i--) {
            var p = parts[i].trim();
            if (p === "no-resolve") continue;
            targetIdx = i;
            break;
        }
        if (targetIdx <= 0) return rule;
        var originalTarget = parts[targetIdx].trim();
        if (BUILTIN_TARGETS.has(originalTarget)) return rule;
        var resolved = _resolveTarget(originalTarget, enabledSet, fallbackMap);
        if (resolved !== originalTarget) {
            parts[targetIdx] = resolved;
            return parts.join(",");
        }
        return rule;
    });
...
function _cleanProxiesList(proxies, enabledSet, allGroupNames) {
    if (!Array.isArray(proxies)) return proxies;
    return proxies.filter(function (p) {
        // Built-in targets are always valid.
        if (BUILTIN_TARGETS.has(p)) return true;
        // If p refers to a known proxy group, keep it only when enabled.
        if (allGroupNames instanceof Set && allGroupNames.has(p)) {
            return enabledSet.has(p);
        }
        // Otherwise p is a real proxy node name - always keep it.
        return true;
    });
}

function _filterEnabledGroups(groups, enabledSet) {
    var allGroupNames = new Set();
    for (var i = 0; i < groups.length; i++) {
        if (groups[i] && groups[i].name) allGroupNames.add(groups[i].name);
    }
    var seen = new Set();
    var result = [];
    for (var i = 0; i < groups.length; i++) {
        ...
        if (g.name !== "Select Node" && g.name !== "Fallback" && !enabledSet.has(g.name)) continue;
        if (seen.has(g.name)) continue;
        seen.add(g.name);
        if (Array.isArray(g.proxies)) {
            g.proxies = _cleanProxiesList(g.proxies, enabledSet, allGroupNames);
        }
        result.push(g);
    }
    return result;
...
function main(config) {
    ...
    const regionalGroups = regions.flatMap(region => {
        if (!enabledSet.has(region.name)) return [];
    ...
    const othersGroup = enabledSet.has("Others") ? {
        ...groupBaseOption,
        "name": "Others",
        "type": "select",
        "include-all": true,
    ...
    if (othersGroup) regionalGroupNames.push("Others");
    // Build the set of all known group names so _cleanProxiesList can
    // distinguish group references from real proxy node names.
    ...
    const standardProxies = _cleanProxiesList([
        "Select Node",
        "Latency Test",
        "Failover",
        "Load Balance (Hash)",
    ...
    // Bettbox: remove disabled built-in functional/auto/block groups before layout.
    // Build the complete set of all known group names BEFORE filtering so
    // references to disabled groups are correctly identified and removed.
    var allKnownGroupNames = new Set();
    for (var gi0 = 0; gi0 < config["proxy-groups"].length; gi0++) {
        if (config["proxy-groups"][gi0] && config["proxy-groups"][gi0].name) {
            allKnownGroupNames.add(config["proxy-groups"][gi0].name);
        }
    }
    for (var ri = 0; ri < regionalGroups.length; ri++) {
        allKnownGroupNames.add(regionalGroups[ri].name);
    }
    allKnownGroupNames.add("Others");

    config["proxy-groups"] = config["proxy-groups"].filter(function (g) {
        if (!g || !g.name) return false;
        if (g.name === "Select Node" || g.name === "Fallback") return true;
        return enabledSet.has(g.name);
    });

    for (var gi = 0; gi < config["proxy-groups"].length; gi++) {
        var gg = config["proxy-groups"][gi];
        if (Array.isArray(gg.proxies)) {
            gg.proxies = _cleanProxiesList(gg.proxies, enabledSet, allKnownGroupNames);
        }
    }

    // Regional groups are split into common and uncommon display sections.
    ...
    const selectNodeIdx = config["proxy-groups"].findIndex(g => g.name === "Select Node");
    if (selectNodeIdx !== -1 && mainstreamGroups.length > 0) {
        config["proxy-groups"].splice(selectNodeIdx + 1, 0, ...mainstreamGroups);
    }

    const fallbackIdx = config["proxy-groups"].findIndex(g => g.name === "Fallback");
    if (fallbackIdx !== -1) {
        const inserts = [];
        if (othersGroup) inserts.push(othersGroup);
        inserts.push(...uncommonRegionalGroups);
        if (inserts.length > 0) {
            config["proxy-groups"].splice(fallbackIdx + 1, 0, ...inserts);
        }
    }
